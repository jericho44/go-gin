...
func TestUserHandler_UpdateUser(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:           "invalid user ID",
			userID:         "invalid",
			requestBody:    handlers.UpdateUserRequest{Name: "John", Email: "john@example.com"},
			mockSetup:      func(m *MockUserService) {},
			expectedStatus: http.StatusBadRequest,
		},
		{
...
// Note: handleServiceError is a private method, so we test it indirectly through public methods
// The error handling is already tested in the other test cases above
