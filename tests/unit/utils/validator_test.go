...
func TestGlobalValidator(t *testing.T) {
	...
	err := utils.ValidateVar("test@example.com", "email")
	assert.NoError(t, err)

	err = utils.ValidateVar("invalid-email", "email")
	assert.Error(t, err)
}
...
func TestValidationEdgeCases(t *testing.T) {
	...
	// Test edge cases for custom validations
	t.Run("Username edge cases", func(t *testing.T) {
		// Exactly 3 characters (minimum)
		err := validator.ValidateVar("abc", "username")
		assert.NoError(t, err)

		// Exactly 30 characters (maximum)
		longUsername := "abcdefghijklmnopqrstuvwxyz1234"
		assert.Equal(t, 30, len(longUsername))
		err = validator.ValidateVar(longUsername, "username")
		assert.NoError(t, err)

		// 31 characters (over limit)
		tooLongUsername := "abcdefghijklmnopqrstuvwxyz12345"
		assert.Equal(t, 31, len(tooLongUsername))
		err = validator.ValidateVar(tooLongUsername, "username")
		assert.Error(t, err)
	})

	t.Run("Password edge cases", func(t *testing.T) {
		// Exactly 8 characters with all requirements
		err := validator.ValidateVar("Abc123!@", "strong_password")
		assert.NoError(t, err)

		// 7 characters (too short)
		err = validator.ValidateVar("Abc12!@", "strong_password")
		assert.Error(t, err)
	})

	t.Run("Phone edge cases", func(t *testing.T) {
		// Exactly 10 digits (minimum)
		err := validator.ValidateVar("1234567890", "phone")
		assert.NoError(t, err)

		// Exactly 15 digits (maximum)
		err = validator.ValidateVar("123456789012345", "phone")
		assert.NoError(t, err)

		// 9 digits (too short)
		err = validator.ValidateVar("123456789", "phone")
		assert.Error(t, err)

		// 16 digits (too long)
		err = validator.ValidateVar("1234567890123456", "phone")
		assert.Error(t, err)
	})
}
