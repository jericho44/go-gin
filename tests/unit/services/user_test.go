...
func TestUserService_GetUserByEmail(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *MockUserRepository) {
				user := &models.User{ID: 1, Name: "John Doe", Email: "john@example.com"}
				m.On("GetByEmail", mock.Anything, "john@example.com").Return(user, nil)
			},
			expectSuccess: true,
		},
		{
			...
			setupMock: func(m *MockUserRepository) {
				m.On("GetByEmail", mock.Anything, "notfound@example.com").Return(nil, errors.New("user not found"))
			},
			expectedError: "user not found",
		},
		{
...
func TestUserService_ListUsers(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(m *MockUserRepository) {
				users := []*models.User{}
				m.On("List", mock.Anything, 10, 0).Return(users, nil) // DefaultLimit = 10
				m.On("Count", mock.Anything).Return(int64(0), nil)
			},
			expectSuccess: true,
			expectedPage:  1,  // DefaultPage = 1
			expectedLimit: 10, // DefaultLimit = 10
		},
		{
			...
			setupMock: func(m *MockUserRepository) {
				users := []*models.User{}
				m.On("List", mock.Anything, 100, 0).Return(users, nil) // MaxPageSize = 100
				m.On("Count", mock.Anything).Return(int64(0), nil)
			},
			expectSuccess: true,
			expectedPage:  1,
			expectedLimit: 100, // MaxPageSize = 100
		},
	}
...
func TestUserService_UserExists(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := services.NewUserService(mockRepo)
			exists, err := service.UserExists(context.Background(), tt.userID)
...
func TestUserService_ValidationMethods(t *testing.T) {
	// Note: These are private methods, so we test them indirectly through public methods
	// The validation logic is already tested in the CreateUser and UpdateUser tests above
	...
	t.Run("email normalization", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, "john@example.com", result.Email) // Should be normalized to lowercase

		mockRepo.AssertExpectations(t)
	})

	t.Run("name trimming", func(t *testing.T) {
		mockRepo := &MockUserRepository{}
		mockRepo.On("ExistsByEmail", mock.Anything, "john@example.com").Return(false, nil)
		mockRepo.On("Create", mock.Anything, mock.AnythingOfType("*models.User")).Return(nil)
		...
		result, err := service.CreateUser(context.Background(), user)
		assert.NoError(t, err)
		assert.Equal(t, "John Doe", result.Name) // Should be trimmed
